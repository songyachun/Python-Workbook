...
class SprintHandler(WebSocketHandler):
    ...
    def on_message(self, message):
        """ Broadcast update to other interested clients """
...
class ScrumApplication(Application):
    def __init__(self, **kwargs):
        routes = [(r'/(?P<sprint>[0-9]+)', SprintHandler), ]
...
if __name__ == "__main__":
    parse_command_line()
    applictaion = ScrumApplication(debug=options.debug)
    server = HTTPServer(applictaion)
